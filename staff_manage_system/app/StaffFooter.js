...
class StaffFooter extends Component {
    handlerAddClick(evt) {
        evt.preventDefault();
        let item = {};
        let addForm = findDOMNode(this.refs.addForm);
        ...
        if (item.name == '' || item.age == '' || item.descrip == '') {
            let tips = findDOMNode(this.refs.tipsUnDone);
        ...
        if (!numReg.test(item.age) || parseInt(item.age) > 150) {
            let tips = findDOMNode(this.refs.tipsUnAge);
            tips.style.display = 'block';
            setTimeout(function() {
                tips.style.display = 'none';
            }, 1000);
        ...
        //此处应在返回添加成功信息后确认
        let tips = findDOMNode(this.refs.tips);
        tips.style.display = 'block';
        setTimeout(function() {
            tips.style.display = 'none';
        }, 1000);
